...
namespace SimpleYolo{
    ...
    */
    ...
    Infer* create_infer(const char* engine_file, Type type, int gpuid, float confidence_threshold, float nms_threshold){
        std::string engine_file_str = engine_file;
        YoloTRTInferImpl* instance(new YoloTRTInferImpl());
        instance->startup(engine_file, type, gpuid, confidence_threshold, nms_threshold);
        INFO("succeed to create infer");
        return instance;
    }
    ...
    bool compile(
        ...
        // convert data type
        std::string source_onnx_str = source_onnx;
        std::string saveto_str = saveto;
        std::string int8_images_folder_str = int8_images_folder;
        std::string int8_entropy_calibrator_cache_file_str = int8_entropy_calibrator_cache_file;
        ...
        if (mode == Mode::INT8) {
            if (!int8_entropy_calibrator_cache_file_str.empty()) {
                if (exists(int8_entropy_calibrator_cache_file_str)) {
                    entropyCalibratorData = load_file(int8_entropy_calibrator_cache_file_str);
                    if (entropyCalibratorData.empty()) {
                        INFOE("entropyCalibratorFile is set as: %s, but we read is empty.", int8_entropy_calibrator_cache_file_str.c_str());
                        return false;
                    }
            ...
            if (hasEntropyCalibrator) {
                if (!int8_images_folder_str.empty()) {
                    INFOW("int8_images_folder is ignore, when int8_entropy_calibrator_cache_file is set");
                }
            }
            else {
                entropyCalibratorFiles = glob_image_files(int8_images_folder_str);
                if (entropyCalibratorFiles.empty()) {
                    INFOE("Can not find any images(jpg/png/bmp/jpeg/tiff) from directory: %s", int8_images_folder_str.c_str());
                    return false;
                }
        ...
        INFO("Compile %s %s.", mode_string(mode), source_onnx_str.c_str());
        ...
        if (!onnxParser->parseFromFile(source_onnx_str.c_str(), 1)) {
            INFOE("Can not parse OnnX file: %s", source_onnx_str.c_str());
            return false;
        }
        ...
        if (mode == Mode::INT8) {
            ...
            if (hasEntropyCalibrator) {
                INFO("Using exist entropy calibrator data[%d bytes]: %s", entropyCalibratorData.size(), int8_entropy_calibrator_cache_file_str.c_str());
                int8Calibrator.reset(new Int8EntropyCalibrator(
                    entropyCalibratorData, calibratorDims, int8process
                ));
            }
            else {
                INFO("Using image list[%d files]: %s", entropyCalibratorFiles.size(), int8_images_folder_str.c_str());
                int8Calibrator.reset(new Int8EntropyCalibrator(
        ...
        if (mode == Mode::INT8) {
            if (!hasEntropyCalibrator) {
                if (!int8_entropy_calibrator_cache_file_str.empty()) {
                    INFO("Save calibrator to: %s", int8_entropy_calibrator_cache_file_str.c_str());
                    save_file(int8_entropy_calibrator_cache_file_str, int8Calibrator->getEntropyCalibratorData());
                }
                else {
        ...
        return save_file(saveto_str, seridata->data(), seridata->size());
    }


    void show_boxes(Box* boxes, int length){
        for (int i = 0; i < length; i++){
            INFO("bbox-%d, (left, top, right, bottom): (%f, %f, %f, %f)", 
                i, boxes[i].left, boxes[i].top, boxes[i].right, boxes[i].bottom
            );
        } 
    }

    void show_mat_shape(const cv::Mat& image){
        INFO("cols: %d, rows: %d", image.cols, image.rows);
        cout << "L= (numpy)" << endl << format(image, Formatter::FMT_NUMPY) << endl << endl;
    }

    Prediction* predict(Infer* engine, const cv::Mat& image){
        shared_future<SimpleYolo::BoxArray> boxes_vec_future;
        boxes_vec_future = engine->commit(image);
        auto boxes_vec = boxes_vec_future.get();
        // cout << "tensorrt prediction result:" << endl;
        Box* boxes = boxes_vec.data();
        int length = boxes_vec.size();
        float **results = new float*[length];
        for(int i = 0; i < length; i++){
            // printf("left: %f, top: %f, right: %f, bottom: %f, label: %d, score: %f\n", 
            // boxes[i].left, boxes[i].top, boxes[i].right, boxes[i].bottom, 
            // boxes[i].class_label, boxes[i].confidence);
            results[i] = new float[6];
            results[i][0] = boxes[i].left;
            results[i][1] = boxes[i].top;
            results[i][2] = boxes[i].right;
            results[i][3] = boxes[i].bottom;
            results[i][4] = (float)boxes[i].class_label;
            results[i][5] = boxes[i].confidence;
        }
        Prediction pred = Prediction(results, length);
        Prediction* pred_ptr = &pred;
        return pred_ptr;
    }

    void reset_engine(Infer* engine){
        delete engine;
        engine = NULL;
    }
};